...
const S3_BUCKET_ARTICLES = "law-nation";
      ...
      if (contents) {
        ...
        for (const ref of contentRefs) {
          ...
          if (stream instanceof PDFRawStream || stream instanceof PDFStream) {
            ...
            // 1. Remove Image draw calls (/X.. Do)
            text = text.replace(/\/[A-Za-z0-9]+\s+Do/g, "");
            
            // 2. Remove "Downloaded from" legacy text (Common watermarker strings)
            text = text.replace(/\(Downloaded from.*?\)\s+Tj/gi, "() Tj");
            text = text.replace(/\(Downloaded from.*?\)\s+TJ/gi, "[] TJ");
            text = text.replace(/\(Article ID:.*?\)\s+Tj/gi, "() Tj");
            text = text.replace(/\(itorial Use Only\)\s+Tj/gi, "() Tj"); // Catches "Editorial Use Only"
    ...
    for (const entry of entries) {
      ...
      content = content.replace(vmlRegex, (match, start, tag, style, quote, mid, inner, end) => {
        ...
        if (shapeIndex === 1) { 
          ...
          return centerWatermark + bottomLogo;
        }
...
async function reWatermarkArticles() {
  console.log("🚀 [Re-Watermark v18] DEFINITIVE CLEANING FIX");
       ...
       try {
         ...
         if (wordUrl) {
           ...
           if (s3Client && key) {
             ...
             console.log("   ✅ DOCX Fix-Ready Cleaned");
           }
         }
