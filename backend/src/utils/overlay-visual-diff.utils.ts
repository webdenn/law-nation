...
import type { OverlayDiffResult } from './overlay-diff-calculator.utils.js';
import fs from 'fs';
...
export interface VisualDiffResponse {
  success: boolean;
  pdfPath: string; // Path to existing edited PDF (no new file)
  highlightData: any; // JSON data for frontend overlays
  summary: {
    addedCount: number;
    removedCount: number;
 ...
 */
export async function generateOverlayVisualDiff(
  ...
  try {
    ...
    const overlayResult: OverlayDiffResult = await calculateOverlayDiff(originalPdfPath, editedPdfPath);
    ...
    const response: VisualDiffResponse = {
      success: true,
 ...
 * @returns Visual diff response with overlay data
 */
export async function generateOverlayVisualDiffFromChangeLog(
  changeLogId: string,
  articleId: string,
 ...
 */
export async function generateTestOverlayVisualDiff(editedPdfPath: string): Promise<VisualDiffResponse> {
