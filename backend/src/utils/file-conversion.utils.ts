...
async function uploadToSupabase(
  ...
  // Extract path from original URL and modify extension
  // Use a fallback if originalUrl is not a valid URL (e.g. local file path)
  let originalFileName = `file-${Date.now()}`;
  try {
     if (isUrl(originalUrl)) {
        const urlPath = new URL(originalUrl).pathname;
        const pathParts = urlPath.split('/').filter(part => part.length > 0);
        originalFileName = pathParts[pathParts.length - 1] || originalFileName;
     } else {
        originalFileName = path.basename(originalUrl);
     }
  } catch (e) {
     console.warn("Could not parse original URL for filename, using default.");
  }
...
export async function convertWordToPdf(
  ...
  try {
    ...
    // --- UPDATED: Save the file via Supabase (Prod) or Local Uploads (Dev) ---
