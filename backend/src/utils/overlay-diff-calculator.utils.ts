...
export interface HighlightData {
  ...
  text: string;
  page: number;
  searchText: string; // Text to search for positioning
  color: string;
  opacity: number;
}
...
export interface OverlayDiffResult {
  ...
  editedPdfPath: string; // Path to existing edited PDF (no new file)
}
 ...
 */
export async function calculateOverlayDiff(
  originalPdfPath: string,
  editedPdfPath: string
): Promise<OverlayDiffResult> {
  ...
  try {
    ...
    const result: OverlayDiffResult = {
      highlights,
      summary: {
 ...
 */
export async function generateTestHighlights(editedPdfPath: string): Promise<OverlayDiffResult> {
