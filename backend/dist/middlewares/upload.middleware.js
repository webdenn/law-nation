...
const MAX_DOCUMENT_SIZE = parseInt(process.env.MAX_DOCUMENT_SIZE_BYTES ?? String(10 * 1024 * 1024), 10); // default 10MB
...
const pdfOnlyFileFilter = (req, file, cb) => {
    if (file.mimetype === "application/pdf")
        ...
        cb(new Error("Only PDF files are allowed for editor uploads"), false);
};
// For editor uploads (DOCX only) - STRICT: Only modern .docx format
const docxOnlyFileFilter = (req, file, cb) => {
...
const MAX_IMAGE_SIZE = parseInt(process.env.MAX_IMAGE_SIZE_BYTES ?? String(5 * 1024 * 1024), 10); // default 5MB
 ...
 */
async function addUploadWatermark(filePath, mimetype) {
...
async function uploadBufferToSupabase(buffer, originalname, mimetype, fileType = 'pdf') {
    if (!supabase) {
    ...
    if (isLocal) {
        ...
        supabaseMemoryDocument.single("document")(req, res, async (err) => {
            ...
            try {
                ...
                const { url, storageKey } = await uploadBufferToSupabase(watermarkedBuffer, file.originalname, file.mimetype);
                req.fileUrl = url;
...
export const uploadPdfOnly = (req, res, next) => {
    if (isLocal) {
...
export const uploadMultiplePdfs = (fieldName = "pdfs", maxCount = 5) => {
    return (req, res, next) => {
        if (isLocal) {
            localUploadDocument.array(fieldName, maxCount)(req, res, async (err) => {
                if (err)
                    return res.status(400).json({ error: err.message });
            ...
            supabaseMemoryDocument.array(fieldName, maxCount)(req, res, async (err) => {
                if (err)
                    return res.status(400).json({ error: err.message });
                const files = req.files ?? [];
                if (!files.length)
...
export const uploadOptionalPdf = (req, res, next) => {
    if (isLocal) {
        localUploadDocument.single("pdf")(req, res, async (err) => {
            if (err)
                return res.status(400).json({ error: err.message });
            if (req.file) {
                try {
        ...
        supabaseMemoryDocument.single("pdf")(req, res, async (err) => {
            if (err)
                return res.status(400).json({ error: err.message });
            const file = req.file;
            if (!file) {
...
export const uploadEditorFiles = (req, res, next) => {
        ...
        upload.fields([
            ...
            try {
                ...
                if (files.document && files.document[0]) {
                    ...
                    console.log(`✅ [Upload] Watermark added, uploading to Supabase...`);
                    // Upload watermarked buffer to Supabase
                    const { url, storageKey } = await uploadBufferToSupabase(watermarkedBuffer, docFile.originalname, docFile.mimetype, 'pdf');
                    req.fileMeta = { url, storageKey };
                }
                else {
                    return res.status(400).json({ error: "Corrected document file required" });
                ...
                if (files.editorDocument && files.editorDocument[0]) {
                    ...
                    console.log(`✅ [Upload] Watermark added, uploading to Supabase...`);
                    // Upload watermarked buffer to Supabase
                    const { url } = await uploadBufferToSupabase(watermarkedBuffer, editorDocFile.originalname, editorDocFile.mimetype, 'pdf');
        ...
        if (isLocal) {
            ...
            supabaseMemoryImage.array(fieldName, maxCount)(req, res, async (err) => {
                ...
                try {
                    const out = [];
                    for (const f of files) {
                        const { url, storageKey } = await uploadBufferToSupabase(f.buffer, f.originalname, f.mimetype, 'image');
                        out.push({ url, storageKey });
                    }
                    req.fileUrls = out;
                    next();
...
export const uploadOptionalImage = (req, res, next) => {
    if (isLocal) {
        localUploadImage.single("image")(req, res, (err) => {
            if (err)
                return res.status(400).json({ error: err.message });
...
export const uploadArticleFiles = (req, res, next) => {
    if (isLocal) {
        const upload = multer({
            storage: multer.diskStorage({
                destination: (req, file, cb) => {
        ...
        upload.fields([
            ...
            try {
                ...
                if (files.pdf && files.pdf[0]) {
                    ...
                    console.log(`✅ [Upload] Watermark added, uploading to Supabase...`);
                    // Upload watermarked buffer to Supabase
                    const { url, storageKey } = await uploadBufferToSupabase(watermarkedBuffer, pdfFile.originalname, pdfFile.mimetype, 'pdf');
                    req.fileMeta = { url, storageKey };
                }
                // Handle thumbnail
                if (files.thumbnail && files.thumbnail[0]) {
                    const thumbFile = files.thumbnail[0];
                    const { url } = await uploadBufferToSupabase(thumbFile.buffer, thumbFile.originalname, thumbFile.mimetype, 'image');
                ...
                if (files.images && files.images.length > 0) {
                    ...
                    for (const imgFile of files.images) {
                        const { url } = await uploadBufferToSupabase(imgFile.buffer, imgFile.originalname, imgFile.mimetype, 'image');
                        imageUrls.push(url);
                    }
...
export const uploadDocxOnly = (req, res, next) => {
    if (isLocal) {
        ...
        supabaseMemoryDocxOnly.single("docx")(req, res, async (err) => {
            ...
            try {
                ...
                const { url, storageKey } = await uploadBufferToSupabase(file.buffer, file.originalname, file.mimetype);
                req.fileUrl = url;
                req.fileMeta = { url, storageKey };
