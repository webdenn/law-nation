 ...
 */
export async function notifyAdminOfEditorApproval(articleId, articleTitle, editorName) {
    try {
        ...
        const notifications = await Promise.all(admins.map(async (admin) => {
            ...
            try {
                await sendEditorApprovalNotificationToAdmin(admin.email, admin.name, articleTitle, editorName, articleId);
 ...
 * Sends BOTH in-app notification AND email
 */
export async function notifyUploaderOfPublication(articleId, articleTitle, uploaderEmail, uploaderName, diffSummary) {
    try {
        const frontendUrl = process.env.FRONTEND_URL || "http://localhost:3000";
        const changeHistoryUrl = `${frontendUrl}/articles/${articleId}/change-history`;
        ...
        try {
            await sendArticlePublishedNotification(uploaderEmail, uploaderName, articleTitle, articleId, diffSummary // Pass diff summary to email
            );
