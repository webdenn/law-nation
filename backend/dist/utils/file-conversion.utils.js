 ...
 */
async function saveConvertedFile(localFilePath, originalUrl, targetExtension) {
    if (useLocalStorage()) {
 ...
 */
async function uploadToSupabase(localFilePath, originalUrl) {
    if (!supabase) {
 ...
 */
export async function convertWordToPdf(wordFilePath) {
    let localWordPath;
    let localPdfPath;
    let isRemote = false;
    ...
    try {
        ...
        if (isRemote) {
            ...
            // Clean up temp files
            if (tempWordPath)
                await fs.unlink(tempWordPath).catch(() => { });
            if (tempPdfPath)
 ...
 */
export async function convertPdfToWord(pdfFilePath) {
    let localPdfPath;
    let localWordPath;
    let isRemote = false;
    let tempPdfPath = null;
 ...
 */
export async function ensureBothFormats(filePath) {
...
//# sourceMappingURL=file-conversion.utils.js.map