...
import { NotFoundError, ForbiddenError, BadRequestError, } from "@/utils/http-errors.util.js";
...
export class ArticleQueryService {
    ...
    async listArticles(filters) {
        ...
        if (assignedEditorId)
            where.assignedEditorId = assignedEditorId;
    ...
    async getArticleHistory(articleId) {
        ...
        const revisionsWithEditor = await Promise.all(revisions.map(async (rev) => {
            let editorInfo = null;
            if (rev.uploadedBy) {
    ...
    async getEditorAssignmentHistory(articleId, userId, userRoles) {
        ...
        const { articleHistoryService } = await import("../article-history.service.js");
...
//# sourceMappingURL=article-query.service.js.map