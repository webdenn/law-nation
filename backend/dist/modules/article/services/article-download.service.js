...
export class ArticleDownloadService {
    ...
    async downloadOriginalDocxWithWatermark(articleId, watermarkData) {
        ...
        const watermarkedBuffer = await addSimpleWatermarkToWord(article.originalWordUrl, watermarkData);
    ...
    async downloadEditorDocxWithWatermark(articleId, watermarkData) {
        ...
        const watermarkedBuffer = await addSimpleWatermarkToWord(article.currentWordUrl, watermarkData);
    ...
    async downloadDiff(changeLogId, userId, userRoles, format = "pdf") {
        ...
        if (format === "word") {
            ...
            buffer = await generateDiffWord(changeLog.diffData, options);
            ...
            buffer = await generateDiffPdf(changeLog.diffData, options);
    ...
    async downloadEditorDocument(changeLogId, userId, userRoles, format = "pdf") {
        ...
        const isAssignedEditor = changeLog.article.assignedEditorId === userId;
        if (!isAdmin && !isAssignedEditor) {
            throw new ForbiddenError("Only admins and assigned editors can download editor documents");
        }
    ...
    async generateVisualDiff(changeLogId) {
        ...
        try {
            ...
            if (updated.count === 0) {
                ...
                if (refreshed?.visualDiffStatus === "READY" &&
        ...
        try {
            ...
            await generateVisualDiffFromChangeLog(changeLogId, changeLog.article.id, changeLog.versionNumber, changeLog.oldFileUrl, changeLog.newFileUrl, fullPath);
...
//# sourceMappingURL=article-download.service.js.map