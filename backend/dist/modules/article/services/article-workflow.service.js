...
import { calculateFileDiff, 
// generateDiffSummary, // COMMENTED: Frontend handles diff calculation
getFileTypeFromPath, } from "@/utils/diff-calculator.utils.js";
import { notifyAdminOfEditorApproval, notifyUploaderOfPublication, } from "@/utils/notification.utils.js";
import { sendEditorAssignmentNotification, sendAuthorAssignmentNotification, sendArticleApprovalNotification, sendEditorReassignmentNotification, } from "@/utils/email.utils.js";
...
export class ArticleWorkflowService {
    ...
    async assignEditor(articleId, data, adminId) {
        ...
        if (!allowedStatuses.includes(article.status)) {
            throw new BadRequestError(`Cannot assign/reassign editor in current status: ${article.status}`);
        }
        ...
        if (isReassignment && oldEditor) {
            sendEditorReassignmentNotification(oldEditor.email, oldEditor.name, article.title, article.id);
            sendEditorAssignmentNotification(newEditor.email, newEditor.name, article.title, article.authorName, article.category, article.id);
            console.log(`ℹ️ [Reassignment] Author not notified (as per policy)`);
        }
        else {
            sendEditorAssignmentNotification(newEditor.email, newEditor.name, article.title, article.authorName, article.category, article.id);
            sendAuthorAssignmentNotification(article.authorEmail, article.authorName, article.title, article.id);
            if (article.secondAuthorEmail && article.secondAuthorName) {
                sendAuthorAssignmentNotification(article.secondAuthorEmail, article.secondAuthorName, article.title, article.id);
            }
        }
        return {
    ...
    async uploadCorrectedPdf(articleId, editorId, data) {
        ...
        if (!validStatuses.includes(article.status)) {
            throw new BadRequestError(`Article status '${article.status}' does not allow corrections. Valid statuses: ${validStatuses.join(', ')}`);
        }
    ...
    async handleDocumentEdit(article, editorId, data) {
        ...
        await prisma.articleChangeLog.create({
            data: {
                ...
                fileType: "DOCX",
                diffData: { type: "document_edit", message: "Document edited by editor using Adobe Services" },
                editedBy: editorId,
                status: "approved",
    ...
    async handleArticleEdit(article, editorId, data) {
        ...
        console.log(`📝 [Change Log] Created version ${versionNumber} for article ${article.id}`);
    ...
    async editorApproveArticle(articleId, editorId) {
        ...
        if (article.status !== "EDITOR_EDITING" &&
        ...
        await notifyAdminOfEditorApproval(articleId, article.title, article.assignedEditor?.name || "Editor");
    ...
    async approveArticle(articleId, userId, userRoles, newPdfUrl) {
        ...
        sendArticleApprovalNotification(article.authorEmail, article.authorName, article.title, article.id);
        if (article.secondAuthorEmail && article.secondAuthorName) {
            sendArticleApprovalNotification(article.secondAuthorEmail, article.secondAuthorName, article.title, article.id);
        }
        return updatedArticle;
    }
    ...
    async adminPublishArticle(articleId, adminId) {
        ...
        if (article.status !== "EDITOR_APPROVED") {
            throw new BadRequestError("Article must be approved by editor before publishing. Current status: " +
                article.status);
        }
    ...
    async publishDocument(article, adminId) {
        ...
        if (article.assignedEditorId) {
            await articleHistoryService.markAsCompleted(article.id, article.assignedEditorId);
        }
        // Notify uploader
        await notifyUploaderOfPublication(article.id, article.title, article.authorEmail, article.authorName, "Document has been professionally edited and published - users will see the corrected version");
    ...
    async publishArticle(article, adminId) {
        ...
        if (article.originalPdfUrl !== article.currentPdfUrl) {
            try {
                ...
                const finalDiff = await calculateFileDiff(article.originalPdfUrl, article.currentPdfUrl);
        ...
        await notifyUploaderOfPublication(article.id, article.title, article.authorEmail, article.authorName, finalDiffSummary);
        if (article.secondAuthorEmail && article.secondAuthorName) {
            await notifyUploaderOfPublication(article.id, article.title, article.secondAuthorEmail, article.secondAuthorName, finalDiffSummary);
        }
        console.log(`✅ [Admin Publish] Article ${article.id} published by admin ${adminId} with text extraction`);
        return {
            article: updatedArticle,
...
//# sourceMappingURL=article-workflow.service.js.map