...
export class ArticleHistoryService {
    ...
    async logReassignment(data) {
        console.log(`📝 [History] Logging reassignment: Article ${data.articleId} | ${data.oldEditorId} → ${data.newEditorId}`);
     ...
     */
    async markAsCompleted(articleId, editorId) {
     ...
     */
    async getArticleEditorHistory(articleId) {
     ...
     */
    async getCurrentEditor(articleId) {
        const activeAssignment = await prisma.articleEditorHistory.findFirst({
            where: {
                articleId,
                status: "active",
     ...
     */
    async getEditorAssignments(editorId, status) {
        const where = { editorId };
        if (status) {
            where.status = status;
        }
     ...
     */
    async getEditorPerformance(editorId) {
        const assignments = await prisma.articleEditorHistory.findMany({
            where: { editorId },
            include: {
                article: {
        ...
        const completedWithTime = assignments.filter((a) => a.status === "completed" && a.unassignedAt && a.assignedAt);
     ...
     */
    async getArticleReassignmentCount(articleId) {
...
//# sourceMappingURL=article-history.service.js.map