...
class AdminDashboardService {
    ...
    async getSummary() {
        try {
            const [totalSubmissions, published, pendingReview, underReview, approved, rejected,] = await Promise.all([
    ...
    async getTimeMetrics(query) {
        try {
            ...
            // Calculate durations
            const submissionToPublished = [];
            const submissionToAssigned = [];
            const assignedToReviewed = [];
            const reviewedToApproved = [];
            const approvedToPublished = [];
            articles.forEach((article) => {
                ...
                if (article.reviewedAt) {
                    ...
                    if (daysToReview !== null)
                        assignedToReviewed.push(daysToReview);
                }
     ...
     */
    async getArticlesTimeline(query) {
        try {
            ...
            const articlesWithTimeline = articles.map((article) => ({
                id: article.id,
                title: article.title,
