  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // 1. Fetch Articles
        const articlesRes = await fetch(`${API_BASE_URL}/api/articles`, { headers });
        const articlesData = await articlesRes.json();
        const articleList = Array.isArray(articlesData) ? articlesData : articlesData.articles || [];
        ...
        // 2. Fetch Editors
        const editorsRes = await fetch(`${API_BASE_URL}/api/users/editors`, { headers });
        if (editorsRes.ok) {
          const editorsData = await editorsRes.json();
          // Backend se array ya data object handle karna
          const editorList = Array.isArray(editorsData) ? editorsData : editorsData.editors || editorsData.data || [];
...
const assignArticle = async (articleId, editorId) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      setArticles(prev =>
        prev.map((a) =>
          a.id === articleId ? { ...a, assignedTo: editorId, status: "In Review" } : a
        )
      );
    ...
    <div className="flex min-h-screen bg-gray-50 flex-col md:flex-row">
      ...
      <aside className="hidden md:flex w-72 bg-red-700 text-white flex-col h-screen sticky top-0 shadow-2xl">
        ...
        <nav className="flex-1 px-4 mt-6 space-y-2">
          ...
          <Link href="/admin/live-database" className="block w-full text-left p-3 hover:bg-red-600 rounded-lg text-red-100 transition-all">Live Database</Link>
        </nav>
        <div className="p-4 border-t border-red-800">
          <button onClick={handleLogout} className="w-full p-2 text-sm bg-red-900 rounded font-bold uppercase hover:bg-black transition-colors">Logout</button>
        </div>
      </aside>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            <div className="bg-gray-50 p-5 border-b flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="flex gap-2 w-full md:w-auto">
                <input type="text" placeholder="Search articles..." className="p-2 border rounded-lg text-xs outline-none focus:border-red-600 w-full" onChange={(e) => setSearchTerm(e.target.value)} />
                <select className="p-2 border rounded-lg text-xs font-bold outline-none cursor-pointer" onChange={(e) => setStatusFilter(e.target.value)}>
                  ...
                  {isLoading ? (
                    ...
                    filteredArticles.map((art) => (
                      <tr key={art.id} className="hover:bg-red-50/30 transition-all">
                        <td className="p-5">
                          <p onClick={() => handlePdfClick(art.pdfUrl)} className="font-bold text-gray-800 underline cursor-pointer hover:text-red-600">{art.title}</p>
                        ...
                        <td className="p-5">
                          <span className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase ${art.status === "Published" ? "bg-green-100 text-green-700" : art.status === "In Review" ? "bg-blue-100 text-blue-700" : "bg-yellow-100 text-yellow-700"}`}>{art.status}</span>
                        </td>
                        <td className="p-5 text-center">
                          ...
                          >
                            ...
                            {editors.map((e) => (
                              <option key={e._id || e.id} value={e._id || e.id}>{e.name || e.email}</option>
                            ))}
                          </select>
                        </td>
