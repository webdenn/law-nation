...
export default function IssueDetailsPage() {
    ...
    const toggleVisibility = async (id, currentVisibility) => {
        try {
            const token = localStorage.getItem("adminToken");
            const res = await fetch(`${NEXT_PUBLIC_BASE_URL}/admin/pdfs/${id}/visibility`, {
                method: "PATCH",
                headers: {
                                    ...
                                    <table className="w-full text-sm text-left">
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                            {filteredIssues.map((item) => (
                                                <tr key={item.id} className={`hover:bg-gray-50 transition ${!item.isVisible ? 'text-gray-400' : 'text-gray-900'}`}>
                                                    <td className="px-4 py-4 font-medium">
                                                        <a
                                                            href={item.watermarkedPdfUrl || item.originalPdfUrl}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className={`hover:underline ${!item.isVisible ? 'text-gray-400' : 'text-red-700'}`}
                                                        >
                                                            {item.title}
                                                        </a>
                                                    </td>
                                                    <td className="px-4 py-4">{item.issue} {item.volume}</td>
                                                    <td className="px-4 py-4">
                                                        {new Date(item.uploadedAt).toLocaleDateString()}
                                                    </td>
                                                    <td className="px-4 py-4">{item.fileSize}</td>
                                                    <td className="px-4 py-4 text-right flex justify-end">
                                                        <button
                                                            onClick={() => toggleVisibility(item.id, item.isVisible)}
                                                            className={`p-2 rounded-full transition-colors ${item.isVisible ? 'text-gray-600 hover:text-gray-800 hover:bg-gray-200' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100'}`}
                                                            title={item.isVisible ? "Hide Issue" : "Show Issue"}
                                                        >
                                                            {item.isVisible ? (
