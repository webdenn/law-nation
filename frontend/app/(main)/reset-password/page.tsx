...
// Reusing the same icons and style as Setup Password
const EyeIcon = () => (
...
function ResetForm() {
    ...
    if (!token) {
        return (
            <div className="w-full max-w-md p-8 text-center border border-red-200 bg-red-50 rounded">
                ...
                <p className="text-red-600 text-sm mt-2">No reset token found. Please check your email again.</p>
            </div>
        );
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${NEXT_PUBLIC_BASE_URL}/api/auth/reset-password`, {
                method: "POST",
            ...
            if (response.ok && data.success) {
                ...
                // Clear storage (security best practice after reset)
                localStorage.clear();
                ...
                toast.error(data.message || data.error || "Failed to reset password. Link may be expired.");
            }
        } catch (error) {
    ...
    return (
        <div className="w-full max-w-md bg-white border border-gray-300 rounded overflow-hidden shadow-sm">
            {/* Header Section */}
            <div className="p-8 text-center border-b border-gray-100">
                <h1 className="text-3xl font-black italic tracking-tighter text-red-700">LAW NATION</h1>
                <p className="text-gray-500 text-sm mt-2 font-medium">Reset your password</p>
